...
prob=0.05;
error        = sqrt( error_outage^2*chi2inv(1-prob,2*N*M)/2 );
%% Simulation loop %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
num_loop = 200; 
load('H_d_all');
load('G_r_all');
...
%     F_ini=ones(N,K)*sqrt(trans_maxpower/(N*K));
    ...
    for n  = 1 : num_iterative
       %%%%%  Optimize F  %%%%%
        [F_1,power_sub,innerflag] = Generate_beamforming_F(N, M, K, H, G, G_error,...
                    F(:,:,n), e(:,n), noise_maxpower, trans_maxpower, rate_min);
        power(n+1)=power_sub;
        F(:,:,n+1)=F_1;
        if innerflag==0
            outerflag=0;

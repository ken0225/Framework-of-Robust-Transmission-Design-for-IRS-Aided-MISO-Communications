...
%     F_ini=ones(N,K)*sqrt(trans_maxpower/(N*K));
    ...
    for n  = 1 : num_iterative
       %%%%%  Optimize F  %%%%%
        [F_1,power_sub,x,innerflag] = Generate_beamforming_F(N, M, K, H, G, H_error,...
              G_error, F(:,:,n), e(:,n), prob, noise_maxpower, trans_maxpower, rate_min);
        power(n+1)=power_sub;
        F(:,:,n+1)=F_1;
        if innerflag==0
            outerflag=0;
